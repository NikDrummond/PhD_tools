...
# --------------------------------------------------------------------------- #
def _linear_pair(g1: jnp.ndarray,
                 g2: jnp.ndarray) -> Tuple[float, float, float]:
        ...
        for j in range(i + 1, m):
            ...
            if use_permutations:
                ...
                p = _perm_test(
                        ...
                        np.concatenate([np.zeros(groups[i].size, dtype=np.int32),
            ...
            if bootstrap_ci:
                boots, (ci_low, ci_high) = _bootstrap(
                    ...
                    n_boot=n_iterations,
                    seed=bootstrap_seed)
            rows.append(dict(group1=labels[i],
                             group2=labels[j],
                             test_statistic=stat,
                             p_value=p,
                             effect_size=eff,
