...
def _compute_group_epdf(
    ...
    use_jax: bool,
    seed: int,
    type: Literal["density", "pmf", "count"] = "density"
) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    """
    Compute histogram-based EPDF or raw counts or discrete PMF and its confidence bounds.
    ...
    use_jax : bool
    seed : int
    type : {'density','pmf','count'}
        - 'density' (default): f_i = count_i / (N * width), so ∫ f(x) dx = 1.
        - 'pmf': p_i = count_i / count.sum(), so ∑ p_i = 1 exactly.
        - 'count': raw counts in each bin.

    Returns
    -------
    f_vals : np.ndarray
        Density, pmf or counts per bin (length = len(x)).
    l : np.ndarray
        Lower CI bound in the same units.
    u : np.ndarray
        Upper CI bound.
    """
    # build edges & counts
    ...
    if ci_method == "dkw":
        ...
        if type == "count":
            # bootstrap returns densities → counts
            low *= N * width
            high *= N * width
        elif type == "pmf":
            # densities→counts→normalize
            low = (low * N * width) / (counts.sum())
            high = (high * N * width) / (counts.sum())
...
def _compute_id_epdf(
    ...
    """
    Compute histogram-based EPDF or PMF or counts for each ID.
    ...
    id_col : str
    x : np.ndarray
    type : {'density','pmf','count'}, default 'density'

    Returns
    -------
    f_id : np.ndarray
        Array of shape (n_bins, n_ids) with density, pmf or counts per ID.
    id_labels : list
        Unique ID values.
    """
    id_labels = df[id_col].unique().tolist()
