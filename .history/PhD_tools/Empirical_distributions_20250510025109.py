...
def compute_ss_jax(
    ...
    """
    Compute within- and total-sum-of-squares.
    Returns (ss_within, ss_total, overall_mean).
    """
    μ  = jnp.mean(y)
...
def compute_logL_jax(
    ss: jnp.ndarray,
    n: int
) -> jnp.ndarray:
    """
    Gaussian log-likelihood under constant variance.
    """
...
def compute_anova_stats_jax(
    ...
    """
    Compute ANOVA F-statistic and omega-squared.
    Returns (F, omega2, overall_mean).
    """
...
def jax_permutation_anova(
    ...
    """
    Compute observed F, permutation distribution of F (Freedman–Lane),
    and observed omega2.
    Returns (F_obs, perm_Fs, omega2_obs).
    """
    F_obs, omega2_obs, μ = compute_anova_stats_jax(y, groups, num_groups)
    resid = y - μ
...
# -----------------------------------------------------------------------------
# NumPy Helper for Stratified Bootstrap of ω²
# -----------------------------------------------------------------------------

def compute_omega2_numpy(y: np.ndarray, groups: np.ndarray) -> float:
    """
    Compute omega-squared via stratified bootstrap logic.
    """
    overall_mean = np.mean(y)
    ss_total     = np.sum((y - overall_mean)**2)
    uniqs        = np.unique(groups)
    ss_within    = sum(
        np.sum((y[groups==g] - np.mean(y[groups==g]))**2)
        for g in uniqs
    )
    n = len(y)
    k = len(uniqs)
...
# -----------------------------------------------------------------------------
# High-level ANOVA: permutation p-value + stratified ω² CI + AIC/BIC
# -----------------------------------------------------------------------------
...
def residual_permutation_anova(
    ...
    # JAX arrays & permutation p-value
    y_jax    = jnp.array(y)
    grp_jax  = jnp.array(labels)
    perm_keys = random.split(random.PRNGKey(seed), num_permutations)

    F_obs_j, perm_Fs_j, omega2_obs_j = jax_permutation_anova(
        y_jax, grp_jax, k, perm_keys
    )
    ...
    # Stratified bootstrap for ω² CI
    rng = np.random.default_rng(seed + 1)
    omega2_boot = []
    for _ in range(num_bootstrap):
        idxs = []
        for g in range(k):
            grp_idx = np.where(labels == g)[0]
            idxs.append(rng.choice(grp_idx, size=len(grp_idx), replace=True))
        boot_idx = np.concatenate(idxs)
        yb       = y[boot_idx]
        groups_b = labels[boot_idx]
        omega2_boot.append(compute_omega2_numpy(yb, groups_b))
    omega2_boot = np.array(omega2_boot)
    ci_lo = float(np.percentile(omega2_boot, (1 - ci) / 2 * 100))
    ci_hi = float(np.percentile(omega2_boot, (1 + ci) / 2 * 100))

    # AIC / BIC
    ssw_j, sst_j, _ = compute_ss_jax(y_jax, grp_jax, k)
...
def posthoc_pairwise_permutation(
    ...
    for i, j in combinations(range(len(uniques)), 2):
        ...
        (obs_diff, perm_diffs, p_raw,
         obs_d,    d_boot,   ci_lo, ci_hi) = jax_pairwise_permutation_cohend(
