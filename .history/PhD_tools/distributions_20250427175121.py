...
@jit
def compute_log_likelihood(y, groups, unique_groups):
    """Compute log-likelihood under group means (Gaussian assumption, common variance)."""
    overall_mean = jnp.mean(y)
    ...
    # Residuals
    group_residuals = y - group_mean[groups]
    overall_residuals = y - overall_mean

    # Sum of squared residuals
    ss_full = jnp.sum(group_residuals ** 2)
    ss_null = jnp.sum(overall_residuals ** 2)

    n = y.shape[0]

    # Log-likelihoods under constant variance assumption
    logL_full = -0.5 * n * (jnp.log(2 * jnp.pi) + jnp.log(ss_full / n) + 1)
    logL_null = -0.5 * n * (jnp.log(2 * jnp.pi) + jnp.log(ss_null / n) + 1)

    return logL_full, logL_null
def permutation_likelihood_ratio_test(
    df, group_col, value_col, num_permutations=1000, seed=42, 
    rank_based=False, return_permutations=False
):
    """
    Perform permutation-based likelihood ratio test for group effects, with optional pseudo-R2 and return of permutation distribution.
    ...
    seed : int, optional
        Random seed for reproducibility.
    rank_based : bool, optional
        If True, perform rank-based LRT.
    return_permutations : bool, optional
        If True, also return permutation distribution of LRT statistics.

    Returns
    -------
    dict
        Dictionary with LRT_statistic, p_value, pseudo_R2, and rank_based.
        If return_permutations=True, also includes 'permuted_LRTs'.
    """
    ...
    logL_full_obs, logL_null_obs = compute_log_likelihood(y, groups, unique_groups)
    LRT_obs = -2 * (logL_null_obs - logL_full_obs)

    # Pseudo-R2 calculation
    pseudo_R2 = 1 - (logL_full_obs / logL_null_obs)
    ...
    def permuted_lrt(k):
        permuted_groups = random.permutation(k, groups)
        logL_full_perm, logL_null_perm = compute_log_likelihood(y, permuted_groups, unique_groups)
        return -2 * (logL_null_perm - logL_full_perm)

    permuted_LRTs = vmap(permuted_lrt)(keys)

    p_value = jnp.mean(permuted_LRTs >= LRT_obs)

    # Prepare result
    result = {
        'LRT_statistic': float(LRT_obs),
        'p_value': float(p_value),
