...
def find_unique_max_assignments_with_advanced_tie_handling(prob_matrix):
    """
    ...
    used_columns = set()  # Keep track of assigned columns
    ...
    for i, row in enumerate(prob_matrix):
        ...
        if max_val == 0:
            ...
            assignments[i] = (i, np.nan)
        elif len(tied_indices) == 1:
            # No tie, assign the single maximum
            col = tied_indices[0]
            if col not in used_columns:  # Ensure column hasn't been used
                assignments[i] = (i, col)
                ...
                assignments[i] = (
                    i,
                    np.nan,
                )  # Row is ignored if the column is already used
        else:
            # Case 2: Tie (non-zero values)
            # Resolve tie by testing all possibilities, considering used columns
            best_score = -np.inf
            best_assignment = None

            for col in tied_indices:
            ...
            # Keep the best assignment for this row, if found
            if best_assignment is not None:
                assignments[i] = (i, best_assignment)
                used_columns.add(best_assignment)
            else:
                assignments[i] = (i, np.nan)  # No valid assignment for this row
...
def unique_col(columns, df, Type, Subtype, sphere_cols, method="prob"):
    tmp_df = df.loc[
        (df.Type == Type) & (df.Subtype == Subtype), ["root_x", "root_y", "root_z"]
    ]
    ...
    if method == "prob":
        ...
        for i in tqdm(columns):
            try:
                prob_array[:, i] = columns[i].point_probability(
                    tmp_df.values, likelihood=True
                )
            except:
                pass
        ...
        prob_array = 1 / cdist(
            tmp_df[["root_x", "root_y", "root_z"]].values,
            sphere_cols,
            metric="euclidean",
        )
    else:
    ...
    df.loc[
        (df.Type == Type) & (df.Subtype == Subtype),
        ["r_unique_x", "r_unique_y", "r_unique_z"],
    ] = coords

    return df


### rayleigh test - from astropy
