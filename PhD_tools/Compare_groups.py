...
def _ss_circular_within(
    groups: List[jnp.ndarray], circmean: Callable[[jnp.ndarray], float] = gj.circmean
) -> float:
...
def _ss_linear(groups: List[jnp.ndarray]) -> Tuple[float, float, int, int]:
    ...
    ss_between = jnp.sum(
        jnp.array([g.size * (jnp.mean(g) - grand) ** 2 for g in valid])
    )
...
def _bootstrap(
    ...
    for i in tqdm(range(n_boot), desc="Bootstrapping", disable=n_boot < 200):
        resamp = [
            jnp.asarray(rng.choice(g, size=g.size, replace=True)) if g.size else g
            for g in groups
        ]
...
def _anova_circular(
    groups: List[jnp.ndarray], circmean=gj.circmean, rvl=_rvl
) -> Tuple[float, float]:
...
def _p_adjust(p: np.ndarray, method: str) -> np.ndarray:
    """Very small p-value adjuster (Bonferroni / Holm)."""
...
# --------------------------------------------------------------------------- #
# Pair-wise helpers (module level – usable everywhere)
# --------------------------------------------------------------------------- #
def _linear_pair(g1: jnp.ndarray, g2: jnp.ndarray) -> Tuple[float, float, float]:
    """F, p, ω² for two linear samples (uses pooled σ²)."""
    n1, n2 = g1.size, g2.size
    ss_b, ss_w, n, _ = _ss_linear([g1, g2])  # k==2 inside
    df_w = n - 2
    ms_w = ss_w / df_w
    pooled_sd = jnp.sqrt(ms_w)
    t = (jnp.mean(g1) - jnp.mean(g2)) / (pooled_sd * jnp.sqrt(1 / n1 + 1 / n2))
    F = t**2
    p = 1 - scipy_f.cdf(float(F), 1, df_w)
    ω2 = float(_omega2(ss_b, ss_w, n, 2))
    return float(F), p, ω2


def _circ_pair(g1: jnp.ndarray, g2: jnp.ndarray) -> Tuple[float, float, float]:
    """F, p, ω² for two circular samples (Watson–Williams analogue)."""
    ss_b = _ss_circular_between([g1, g2])
    ss_w = _ss_circular_within([g1, g2])
    n = int(g1.size + g2.size)
    F = ((n - 2) * ss_b) / (ss_w + 1e-12)
    p = 1 - scipy_f.cdf(float(F), 1, n - 2)
    ω2 = float(_omega2(ss_b, ss_w, n, 2))
    return float(F), p, ω2
...
def pairwise_compare_groups(
    ...
    *,
    ...
    bootstrap_ci: bool = False,
    bootstrap_seed: int | None = None,
    p_adjust: str | None = "holm",  # None | "bonferroni" | "holm"
) -> pd.DataFrame:
    ...
    labels = pd.Series(df[group_col].to_numpy()).astype(str).unique()

    pair_fn = _circ_pair if method == "circular" else _linear_pair

    # ---------- loop over all contrasts ------------------------------------
    rows, m = [], len(groups)
    for i in range(m - 1):
        for j in range(i + 1, m):
            stat, p, eff = pair_fn(groups[i], groups[j])
            ...
            if use_permutations:
                if method == "circular":
                    stat_fn = lambda g: _ss_circular_between(g)
                    observed = _ss_circular_between([groups[i], groups[j]])
                else:  # linear
                    stat_fn = lambda g: _anova_linear(g)[0]
                    observed = stat  # F from _linear_pair

                p = _perm_test(
                    jnp.concatenate([groups[i], groups[j]]),
                    np.concatenate(
                        [
                            np.zeros(groups[i].size, dtype=np.int32),
                            np.ones(groups[j].size, dtype=np.int32),
                        ]
                    ),
                    stat_fn,
                    observed,
            ...
            if bootstrap_ci:
                boots, (ci_low, ci_high) = _bootstrap(
                    [groups[i], groups[j]],
                    eff_fn=lambda g: _omega2(
                        *(
                            _ss_linear(g)
            ...
            rows.append(
                dict(
                    ...
                    p_value=p,
                    effect_size=eff,
                    ci_low=ci_low if bootstrap_ci else None,
                    ci_high=ci_high if bootstrap_ci else None,
                )
            )
    ...
    if p_adjust:
        out["p_adj"] = _p_adjust(out["p_value"].to_numpy(), p_adjust)
    else:
